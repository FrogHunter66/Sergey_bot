...
async def add_test2(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.answer(f"""🔓Укажите код, по которому будет осуществлен доступ к тестам и опросам вашего мероприятия, <b>код должен быть пятизначным числом</b>.
